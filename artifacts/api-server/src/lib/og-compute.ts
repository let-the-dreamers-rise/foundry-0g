...
/**
 * Calls a 0G Compute TEE-attested inference broker (OpenAI-compatible
 * /v1/chat/completions). Returns null when the broker is not configured —
 * the caller then falls back to a simulated response.
 *
 * Required env: OG_COMPUTE_BROKER_URL (optional: OG_COMPUTE_API_KEY)
 */
export async function callOgCompute(
  ...
  try {
    ...
    return {
      response: content,
      teeAttestationRef: `tee_real_${data.id ?? Date.now()}_${modelRef.slice(2, 10)}`,
      real: true,
      brokerUrl,
    };
  } catch (err) {
    log.warn({ err: String(err), brokerUrl }, "0G Compute broker call failed — falling back");
    return null;
  }
}
...
function buildSystemPrompt(category: string): string {
  const prompts: Record<string, string> = {
    "customer-support":
      "You are a helpful customer support AI fine-tuned on domain-specific support data. Provide clear, empathetic, actionable responses.",
    "creative-writing":
      "You are a creative writing AI fine-tuned to produce imaginative, high-quality prose. Be vivid, descriptive, engaging.",
    "code-assistant":
      "You are a coding assistant AI fine-tuned on software engineering data. Provide precise, well-explained code solutions.",
    finance:
      "You are a financial analysis AI fine-tuned on market data. Provide balanced, data-driven insights.",
    medical:
  ...
  return prompts[category] ?? "You are a specialized AI assistant. Provide helpful, accurate responses.";
}
